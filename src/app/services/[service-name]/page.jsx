...
export default function ServicePage({ params }) {
  ...
  return (
    <main>
      ...
      {/* Sub Services */}
      <section className="max-w-6xl mx-auto px-4 space-y-16 pb-20">
        {service.subServices.map((sub, i) => (
          <div
            key={i}
            className={`flex flex-col md:flex-row ${
              i % 2 === 1 ? "md:flex-row-reverse" : ""
            } items-center bg-gray-900 rounded-xl shadow p-6 gap-6`}>
            {/* Left: Card */}
            <div className="md:w-1/2">
              ...
              />
              <h3 className="text-2xl font-bold text-blue-900">{sub.name}</h3>
              ...
              <ul className="flex flex-wrap gap-2 mb-6 p-0">
                {sub.features.map((feature, i) => (
                  <li key={i}>
                    <span className="inline-flex items-center gap-2 border border-gray-800 bg-[#ffbd59] text-black px-4 py-2 rounded-full text-sm font-medium">
                      <i
                        className={`bi ${
                          i === 0
            ...
            <div className="md:w-1/2">
              <p className="text-lg mb-4">{sub.description}</p>
              <hr className="my-4" />
